...
#table S10. (Wilcoxon rank sums test for droughts vs. other slow onset hazards)-------------------
      #this was formerly table S9b.
      s9bdr <- finaldata %>% filter(time %in% c(30,60) & H.5. %in% c("Dr"))
      s9bnodr_slow <- finaldata %>% filter(time %in% c(30,60) & H.7. %in% c(1) & !H.5. %in% c("Dr")) %>% mutate(H.5. = ifelse(!H.5. %in% c("Dr"), "OTHER", H.5.))
      ...
      ranks_Slow <- rank(c(s9bnodr_slow$H.8.))
      sum_ranks_Sl <- sum(ranks_Slow)
      mean_ranks_Sl <- mean(ranks_Slow)
      
      table.s9b1 <- data.frame(Hz_type = c("Dr", "NoDr_Slow", "Total"), Num_events =
                                 c(nrow(s9bdr),nrow(s9bnodr_slow),sum(nrow(s9bdr),nrow(s9bnodr_slow))), 
                               Mean_rank = c(mean_ranks_Dr,mean_ranks_Sl,NA), sum_ranks = c(sum_ranks_Dr,sum_ranks_Sl,NA))
      writexl::write_xlsx(table.s9b1, "S9b-1")

      #part 2: calculate the actual U and W scores
      combined_data1 <- c(s9bdr$H.8., s9bnodr_slow$H.8.)
      group_indicator <- c(rep("Dr", length(s9bdr$H.8.)), rep("Sl", length(s9bnodr_slow$H.8.)))
      ...
      dr <- c(s9bdr$H.8.)
      sl <- c(s9bnodr_slow$H.8.)
      table.s9b3 <- data.frame(Hz_type = c("Droughts", "Slow"), Num_events = c(nrow(s9bdr),nrow(s9bnodr_slow)),
